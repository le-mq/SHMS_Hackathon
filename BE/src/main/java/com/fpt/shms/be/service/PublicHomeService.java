...
public class PublicHomeService {
    ...
    public PublicHomeResponse getHomeData() {
        ...
        // 3. Active announcements (newest first)
        ...
        return new PublicHomeResponse(contests, tracks, announcements, stats, universities, scopes);
    }
    ...
    public List<Map<String, Object>> getPublishedLeaderboards() {
        try {
            return rankingResultRepository.findAll().stream()
                    .map(rr -> {
                        Map<String, Object> map = new HashMap<>();
                        map.put("contestId", rr.getRound() != null && rr.getRound().getContest() != null ? rr.getRound().getContest().getId() : null);
                        map.put("contestName", rr.getRound() != null && rr.getRound().getContest() != null ? rr.getRound().getContest().getName() : "Unknown Contest");
                        map.put("roundName", rr.getRound() != null ? rr.getRound().getPhaseName() : null);
                        map.put("publishedAt", rr.getDatePublishedAt());
                        map.put("teamName", rr.getTeam() != null ? rr.getTeam().getName() : null);
                        map.put("categoryName", rr.getTeam() != null && rr.getTeam().getCategory() != null ? rr.getTeam().getCategory().getName() : "General");
                        map.put("rank", rr.getRankNo());
                        map.put("finalScore", rr.getFinalScore());
                        map.put("status", rr.getQualificationStatus());
                        return map;
                    })
                    .filter(java.util.Objects::nonNull)
                    .toList();
