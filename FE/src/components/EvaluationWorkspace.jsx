...
const EvaluationWorkspace = () => {
        ...
        const fetchEvalData = async () => {
            try {
                ...
                if (teamId === 'preview' && roundId) {
                    fetchUrl = `import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/judge/evaluation-data/0?roundId=${roundId}`;
                } else {
                    fetchUrl = `import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/judge/evaluation-data/${teamId}`;
                    if (roundId) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const token = localStorage.getItem('shms_token');
            const response = await fetch('import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/judge/submit-score', {
                method: 'POST',
                headers: {
