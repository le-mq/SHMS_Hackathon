...
const API_PUBLIC = 'import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/public';
...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchPendingInvitations = async () => {
            try {
                const token = localStorage.getItem('shms_token');
                const res = await fetch(`import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/student/teams/invitations/pending`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                if (res.ok) {
    ...
    const handleRespondInvitation = async (invitationToken, action) => {
        try {
            const token = localStorage.getItem('shms_token');
            const res = await fetch(`import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/student/teams/invitations/respond`, {
                method: 'POST',
