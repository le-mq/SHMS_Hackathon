...
const NavbarJudge = () => {
    ...
    const userEmail = localStorage.getItem('shms_user') || 'judge@shms.edu.vn';
    ...
    return (
        ...
        <nav className="evaluator-nav">
            <div className="evaluator-nav-brand" onClick={() => handleNav('/judge/workspace')} style={{cursor:'pointer'}}>
            ...
            <div className="evaluator-nav-links">
                {JUDGE_LINKS.map(link => (
                    <div
                        key={link.path}
                        className={`evaluator-nav-link ${(link.path === '#leaderboard' ? isLeaderboard : (location.pathname === link.path && !isLeaderboard)) ? 'active' : ''}`}
