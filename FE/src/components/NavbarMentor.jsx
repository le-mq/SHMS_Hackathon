...
import './Navbars.css';
import { LeaderboardContent } from './LeaderDashboard';
...
const NavbarMentor = () => {
    ...
    const userEmail = localStorage.getItem('shms_user') || 'mentor@shms.edu.vn';
    const username  = localStorage.getItem('shms_fullname_' + userEmail) || localStorage.getItem('shms_fullname') || localStorage.getItem('shms_user') || 'Mentor';
    ...
    const hasJudgeRole = allRoles.includes('JUDGE');

    const handleSwitchRole = async (targetRole) => {
        try {
            const token = localStorage.getItem('shms_token');
            const response = await fetch('http://localhost:8080/api/v1/auth/switch-role', {
                method: 'POST',
                headers: {
                ...
                <div className="nav-user-profile" onClick={() => setShowDropdown(!showDropdown)}>
                    ...
                    {showDropdown && (
                        <div className="nav-dropdown">
                            ...
                            {hasJudgeRole && (
                                <div className="nav-dropdown-item" onClick={() => handleSwitchRole('JUDGE')}>
