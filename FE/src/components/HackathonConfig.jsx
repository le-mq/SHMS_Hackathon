...
const RemoveButton = ({ onClick, title }) => (
    <button type="button" onClick={onClick} title={title} className="hc-remove-btn">
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, setStatus }) => {
            ...
            try {
                ...
                fetchContests();
            } catch (err) { setStatus({ error: err.message || 'Failed to connect to server' });
                    ...
                    <div className="hc-content">
                                ...
                                <div className="hc-section">
                                    ...
                                    <div className="hc-grid-2">
                                        <div className="hc-field">
                                            ...
                                            <input className={`hc-input${formik.touched.name && formik.errors.name ? ' is-invalid' : ''}`} name="name" value={formik.values.name} onChange={formik.handleChange} onBlur={formik.handleBlur} disabled={isClosedContest} placeholder="e.g. FPT Hackathon 2026" />
                                        ...
                                        <div className="hc-field">
                                            ...
                                            <input className={`hc-input${formik.touched.theme && formik.errors.theme ? ' is-invalid' : ''}`} name="theme" value={formik.values.theme} onChange={formik.handleChange} onBlur={formik.handleBlur} disabled={isClosedContest} placeholder="e.g. AI & Web3 Innovation" />
                                    ...
                                    <div className="hc-grid-4" style={{ marginTop: 20 }}>
                                        ...
                                        <div className="hc-field" style={{ gridColumn: 'span 2' }}>
                                            ...
                                            <input className={`hc-input${formik.touched.location && formik.errors.location ? ' is-invalid' : ''}`} name="location" value={formik.values.location} onChange={formik.handleChange} onBlur={formik.handleBlur} disabled={isClosedContest} />
                                                ...
                                                <div key={round.id} style={{ marginBottom: 16 }}>
                                                    ...
                                                    <div className="hc-grid-2" style={{ marginBottom: 20 }}>
                                                        <div className="hc-field">
                                                            ...
                                                            <input className={`hc-input${formik.touched.rounds?.[rIdx]?.phaseName && formik.errors.rounds?.[rIdx]?.phaseName ? ' is-invalid' : ''}`} name={`rounds[${rIdx}].phaseName`} value={round.phaseName} onChange={formik.handleChange} onBlur={formik.handleBlur} disabled={isClosedContest} />
                        ...
                        {valItems.length > 0 && (
                            <div className="hc-val-summary">
                                <div className="hc-val-summary-header">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> Validation Summary
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                    {valItems.map((item, i) => (
                                        <div key={i} className={`hc-val-item ${item.type}`} onClick={() => item.field && focusField(item)}>
                                            {item.type === 'error' ? (
                                            ...
                                            <strong>{item.field.replace(/\[(\d+)\]/g, (m, d) => ` ${parseInt(d)+1}`).replace(/\./g, ' - ').replace(/([A-Z])/g, ' $1').replace(/^./, x => x.toUpperCase())}:</strong>&nbsp;{item.msg}
                                        </div>
                                    ))}
                                </div>
                            </div>
