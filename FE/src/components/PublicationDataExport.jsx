...
const PublicationDataExport = () => {
    ...
    const handlePublishAnnouncement = async () => {
        ...
        try {
            const token = localStorage.getItem('shms_token');
            const res = await fetch('import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/admin/contests/announcements', {
                method: 'POST',
                headers: {
    ...
    const handleExport = (type) => {
        ...
        window.open(`import.meta.env.VITE_API_BASE_URL || "import.meta.env.VITE_API_BASE_URL || "http://localhost:8080/api/v1""/admin/results/export-csv?type=${type}${contestParam}&token=${token}`, '_blank');
    };
