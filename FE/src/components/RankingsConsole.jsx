...
const RankingsConsole = () => {
    ...
    useEffect(() => {
        // Fetch Rounds for selected Contest
        const fetchRounds = async () => {
            if (!selectedContestId) return;
            try {
                const token = localStorage.getItem('shms_token');
                const headers = { 'Authorization': `Bearer ${token}` };
                const res = await fetch(`http://localhost:8080/api/v1/admin/contests/${selectedContestId}`, { headers });
                if (res.ok) {
                    ...
                    if (data.tracks) {
                        data.tracks.forEach(t => {
                            if (t.rounds) {
                                t.rounds.forEach(r => {
                                    if (r.phaseName) phaseSet.add(r.phaseName);
                                });
                            }
                        });
                    }
                    const dynamicRounds = Array.from(phaseSet);
                    if (dynamicRounds.length > 0) {
                        setRounds(dynamicRounds);
                        setRound(dynamicRounds[0]);
                    } else {
                        // Fallback if no rounds configured
                        setRounds(['Phase 01: Screening', 'Phase 02: Semi-Final', 'Phase 03: Final']);
                        setRound('Phase 01: Screening');
                    }
                }
            } catch (err) {
                console.error(err);
    ...
    useEffect(() => {
        // Fetch Readiness whenever contest or round changes
        const fetchReadiness = async () => {
            if (!selectedContestId || !round) return;
            try {
                const token = localStorage.getItem('shms_token');
                const res = await fetch(`http://localhost:8080/api/v1/admin/rankings/readiness?contestId=${selectedContestId}&roundName=${encodeURIComponent(round)}`, {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const token = localStorage.getItem('shms_token');
            const response = await fetch('http://localhost:8080/api/v1/admin/rankings/process', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                body: JSON.stringify({ contestId: selectedContestId, round, topN: parseInt(topN) })
            });
    ...
    const handlePublish = async () => {
        ...
        try {
            const token = localStorage.getItem('shms_token');
            const response = await fetch('http://localhost:8080/api/v1/admin/rankings/publish', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                body: JSON.stringify({ contestId: selectedContestId, roundName: round, topN: parseInt(topN) })
            });
    ...
    const maxBar = Math.max(...readinessData.summary.bars, 1); // prevent division by zero
    ...
    return (
        <div className="rankings-container">
            ...
            <div className="rankings-content">
                ...
                {/* Readiness Banner — BR-RNK-02 Guard */}
                ...
                <div className="rankings-grid">
                    ...
                    <div className="config-card">
                        ...
                        />
                        {/* BR-RNK-02: disable until all evaluators finalized */}
                        <button
